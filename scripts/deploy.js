...
if (!creds) {
  Log.error(`Credentials missing for org "${org}."`);
}
Log.info(`Credentials found for org "${org}", starting deploy...`);

var conn = new Jsforce.Connection();
conn.login(creds.username, `${creds.password}${creds.token}`, (err, res) => {
  if (err) {
    Log.error(err);
  }
  deploy(conn);
